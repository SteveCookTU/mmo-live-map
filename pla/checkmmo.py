...
with open("./static/resources/text_species_en.txt",
...
def generate_mass_outbreak_aggressive_path(group_seed, rolls, steps, uniques,
                                           storage, spawns, true_spawns,
    ...
    for step_i, step in enumerate(steps):
        for pokemon in range(1, step + 1):
            ...
            encryption_constant, pid, ivs, ability, gender, nature, shiny = \
                generate_from_seed(fixed_seed, rolls, guaranteed_ivs)
            if not fixed_seed in uniques and (
                    sum(steps[:step_i]) + pokemon + 4) <= true_spawns:
                uniques.add(fixed_seed)
                info = {
...
def aggressive_outbreak_pathfind(group_seed,
    ...
    if sum(_steps) + step < spawns - 4:
        ...
        generate_mass_outbreak_aggressive_path(group_seed, rolls, _steps,
                                               uniques, storage, spawns,
                                               true_spawns, encounters, encsum,
                                               isbonus, isalpha)
...
def next_filtered_aggressive_outbreak_pathfind(reader, group_seed, rolls,
                                               spawns, true_spawns, group_id,
                                               mapcount, isbonus,
                                               isalpha=False):
    """Check the next outbreak advances until an aggressive path to a pokemon that
       passes poke_filter exists"""
    encounters, encsum = get_encounter_table(reader, group_id, mapcount,
                                             isbonus)
    main_rng = XOROSHIRO(group_seed)
    result = []
    advance = -1
...
def get_bonus_seed(reader, group_id, rolls, mapcount, path):
    species = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * mapcount:X}",
        2)
    ...
    if species != 0:
        ...
        group_seed = reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * mapcount + 0x44:X}",
            8)
        max_spawns = reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * mapcount + 0x60:X}",
            4)
        # print(f"Max spawns: {max_spawns}")
        curr_spawns = reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * mapcount + 0x50:X}",
            4)
        main_rng = XOROSHIRO(group_seed)
        for init_spawn in range(4):
            ...
            fixed_rng.next()
            fixed_seed = fixed_rng.next()
            ec, pid, ivs, ability, gender, nature, shiny = generate_from_seed(
                fixed_seed, rolls)
        group_seed = main_rng.next()
        main_rng = XOROSHIRO(group_seed)
        respawn_rng = XOROSHIRO(group_seed)
        ...
        for respawn, step in enumerate(path):
            ...
            respawn_rng = XOROSHIRO(respawn_rng.next())
        bonus_seed = (
                                 respawn_rng.next() - 0x82A2B175229D6A5B) & 0xFFFFFFFFFFFFFFFF
...
def read_mass_outbreak_rng(reader, group_id, rolls, mapcount, bonus_flag):
    ...
    if species != 0:
        ...
        if bonus_flag:
            ...
            max_spawns += 3
        display = next_filtered_aggressive_outbreak_pathfind(reader, group_seed,
                                                             rolls, max_spawns,
                                                             true_spawns,
                                                             group_id, mapcount,
                                                             bonus_flag, False)
...
def get_encounter_table(reader, group_id, mapcount, bonus):
    ...
    encsum = 0
    if bonus:
        enc_pointer = reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * mapcount + 0x2c:X}",
            8)
    else:
        enc_pointer = reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * mapcount + 0x24:X}",
            8)
...
def generate_mass_outbreak_aggressive_path_seed(group_seed, rolls, steps,
                                                uniques, storage, spawns,
                                                true_spawns, encounters, encsum,
                                                true_seed, isbonus=False,
                                                isalpha=False):
    """Generate all the pokemon of an outbreak based on a provided aggressive path"""
    # pylint: disable=too-many-locals, too-many-arguments
    # the generation is unique to each path, no use in splitting this function
    ...
    for step_i, step in enumerate(steps):
        ...
        if ((steps[:step_i] + [pokemon]) not in uniques) and (
                (sum(steps[:step_i]) + pokemon) - respawns >= 0 and (
                sum(steps[:step_i]) - respawns) < 0):
            add1 = steps[:step_i]
...
def aggressive_outbreak_pathfind_seed(group_seed,
    ...
    if sum(_steps) + step < spawns - 4:
        ...
        _steps.append(spawns - sum(_steps) - 4)
        generate_mass_outbreak_aggressive_path_seed(group_seed, rolls, _steps,
                                                    uniques, storage, spawns,
                                                    true_spawns, encounters,
                                                    encsum, true_seed, isbonus,
                                                    isalpha)
...
def next_filtered_aggressive_outbreak_pathfind_seed(reader, group_seed, rolls,
                                                    spawns, true_spawns,
                                                    group_id, mapcount, isbonus,
                                                    true_seed, isalpha=False):
    """Check the next outbreak advances until an aggressive path to a pokemon that
       passes poke_filter exists"""
    encounters, encsum = get_encounter_table(reader, group_id, mapcount,
                                             isbonus)
    ...
    while len(result) == 0 and advance < 1:
        if advance != -1:
            ...
            main_rng.reseed(group_seed)
        advance += 1
        result = aggressive_outbreak_pathfind_seed(group_seed, rolls, spawns,
                                                   true_spawns, encounters,
                                                   encsum, true_seed, isbonus,
                                                   isalpha)
        if result is None:
...
def generate_mass_outbreak_aggressive_path_normal(group_seed, rolls, steps,
                                                  uniques, storage):
    """Generate all the pokemon of an outbreak based on a provided aggressive path"""
    # pylint: disable=too-many-locals, too-many-arguments
    # the generation is unique to each path, no use in splitting this function
...
def aggressive_outbreak_pathfind_normal(group_seed,
    ...
    if sum(_steps) + step < spawns - 4:
        ...
        _steps.append(spawns - sum(_steps) - 4)
        generate_mass_outbreak_aggressive_path_normal(group_seed, rolls, _steps,
                                                      uniques, storage)
        if _steps == get_final(spawns):
            return storage
    return None
...
def next_filtered_aggressive_outbreak_pathfind_normal(group_seed, rolls,
                                                      spawns):
    """Check the next outbreak advances until an aggressive path to a pokemon that
       passes poke_filter exists"""
    main_rng = XOROSHIRO(group_seed)
...
def get_max_spawns(reader, group_id, maps, isbonus):
    if isbonus:
        max_spawns = reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * maps + 0x60:X}",
            4)
    else:
        max_spawns = reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * maps + 0x4c:X}",
            4)
...
def get_map_name(reader, maps):
    mapname = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + 0xb80 * maps - 0x24:X}", 2)
    mapname = f"{mapname:X}"
...
def get_bonus_flag(reader, group_id, maps):
    return True if reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * maps + 0x18:X}",
        1) == 1 else False


def get_normal_outbreak_info(reader, group_id):
    species = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x20 + group_id * 0x50:X}", 2)
    group_seed = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x20 + group_id * 0x50 + 0x38:X}",
        8)
    max_spawns = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x20 + group_id * 0x50 + 0x40:X}",
        8)

    coords = struct.unpack('fff', reader.read_pointer(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x20 + group_id * 0x50 + 0x20:X}",
        12))
...
def read_bonus_pathinfo(reader, paths, group_id, mapcount, rolls, group_seed,
                        map_name, coords):
    isbonus = True
    outbreaks = {}
    max_spawns = 10
    ...
    for t, value in enumerate(paths):
        ...
        # print(f"Seed: {seed:X}")
        extra = [1] * (get_max_spawns(reader, group_id, mapcount, False) - sum(
            value))
        display = next_filtered_aggressive_outbreak_pathfind(reader, seed,
                                                             rolls, max_spawns,
                                                             true_spawns,
                                                             group_id, mapcount,
                                                             isbonus, False)
        ...
        for index in display:
            ...
            display[index][
                "index"] = f"First Round Path: {value} + {extra} Bonus " + \
                           display[index]["index"]
            display[index]["group"] = group_id
            display[index]["mapname"] = map_name
            display[index]["coords"] = coords
            if " " in display[index]["species"] and "-" in display[index][
                "species"]:
...
def get_map_mmos(reader, mapcount, rolls):
    ...
    for i in range(0, 16):
        ...
        display, species = read_mass_outbreak_rng(reader, i, rolls, mapcount,
                                                  False)
        for index in display:
            if index != "index" and index != "description":
                ...
                if " " in display[str(index)]["species"] and "-" in \
                        display[str(index)]["species"]:
                    cutspecies = display[str(index)]["species"].rpartition(' ')[
                        2]
        ...
        if bonus_flag:
            ...
            bonus_seed = next_filtered_aggressive_outbreak_pathfind_seed(reader,
                                                                         group_seed,
                                                                         rolls,
                                                                         max_spawns,
                                                                         true_spawns,
            ...
            result = read_bonus_pathinfo(reader, bonus_seed, i, mapcount, rolls,
                                         group_seed, map_name, coords)
...
def read_normal_outbreaks(reader, rolls):
    ...
    print(f"Rolls: {rolls}")
    for i in range(0, 4):
        species, group_seed, max_spawns, coordinates = get_normal_outbreak_info(
            reader, i)
        if species != 0:
            display = next_filtered_aggressive_outbreak_pathfind_normal(
                group_seed, rolls, max_spawns)
            for index in display:
                if index != "index" and index != "description":
                    form = ''
                    display[str(index)]["group"] = i
                    ...
                    if " " in display[str(index)]["species"] and "-" in \
                            display[str(index)]["species"]:
                        cutspecies = \
                        display[str(index)]["species"].rpartition(' ')[2]
                        form = display[str(index)]["species"].rpartition('-')[2]
                        cutspecies = cutspecies.rpartition('-')[0]
                    elif " " in display[str(index)]["species"]:
                        cutspecies = \
                        ...
                        cutspecies = \
                        display[str(index)]["species"].rpartition('-')[0]
                        form = display[str(index)]["species"].rpartition('-')[2]
                    else:
                        cutspecies = display[str(index)]["species"]
                    # print(f"Species: {display[index]['species']}")
...
def read_group_coordinates(reader, group_id, mapcount):
    ...
    coords = struct.unpack('fff', reader.read_pointer(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + group_id * 0x90 + 0xb80 * mapcount - 0x14:X}",
        12))
...
def teleport_to_spawn(reader, coords):
    ...
    reader.write_pointer(PLAYER_PTR,
